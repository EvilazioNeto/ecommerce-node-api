...
import { CriarProdutosProps, IProduto, RecuperarProdutoProps, StatusProduto } from "./produto.types";
...
class Produto extends Entity<IProduto> implements IProduto {
    ...
    public removerCategoria(categoria: Categoria): Categoria {
        const qtdCategoriaProduto: number = this.quantidadeCategorias();

        if (qtdCategoriaProduto <= Produto.QTD_MINIMA_CATEGORIAS) {
